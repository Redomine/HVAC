...
__title__ = 'Распределение по\nрабочим наборам'
__doc__ = "Распределяет элементы в выбранный рабочий набор"
...
lstPreview =  WorksharingUtils.GetUserWorksetInfo(doc.GetWorksharingCentralModelPath())

worksets = []
worksets_id = []
for Item in lstPreview:
    wkset = worksetTable.GetWorkset(Item.Id)
    worksets.append(wkset.Name)
    worksets_id.append(Item.Id.IntegerValue)
...
check_workset = []
for x in worksets:
    check_workset.append(x)  

workset_todo = SelectFromList('Выберите наполняемый рабочий набор', check_workset)
 

workset_todo_id = worksets_id[worksets.index(workset_todo)]



colFittings = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_DuctFitting)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colPipeFittings = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_PipeFitting)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colPipeCurves = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_PipeCurves)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colCurves = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_DuctCurves)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colTerminals = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_DuctTerminal)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colAccessory = FilteredElementCollector(doc)\
                            .OfCategory(BuiltInCategory.OST_DuctAccessory)\
                            .WhereElementIsNotElementType()\
                            .ToElements()

colPipeAccessory = FilteredElementCollector(doc)\
...
project_todo = SelectFromList('Рабочий набор чего вы хотите изменить?', ['Система воздуховодов','Клапана/Арматура воздуховодов', 'Оборудование', 'Системы трубопроводов', 'Арматура трубопроводов'])

t = Transaction(doc, 'Перенос расходов в оборудование')

t.Start()

def change_system_workset(collection, worksets_id, systems_to_workset, workset_todo_id):
    for element in collection:
        if element.Location:
            if element.LookupParameter('Имя системы'):
                system = element.LookupParameter('Имя системы').AsString()
                for checked_system in systems_to_workset:
                    if checked_system == system:
                        if element.LookupParameter('Рабочий набор'):
                            workset = element.LookupParameter('Рабочий набор')
                            workset.Set(workset_todo_id)
                            
def change_element_workset(collection, worksets_id, elements_to_workset, workset_todo_id):
    for element in collection:
        if element.Location:
            if element.LookupParameter('Имя семейства'):
                family = element.Symbol.Family.Name
                for checked_element in elements_to_workset:
                    if checked_element == family:                  
                        if element.LookupParameter('Рабочий набор'):
                            workset = element.LookupParameter('Рабочий набор')
                            workset.Set(workset_todo_id)
...
if project_todo == 'Система воздуховодов':
    systems = []
    ...
    systems_to_workset = cheklist(systems)
    change_system_workset(colCurves, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colFittings, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colTerminals, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colAccessory, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colEquipment, worksets_id, systems_to_workset, workset_todo_id)
    
elif  project_todo == 'Клапана/Арматура воздуховодов':
    elements = []
    for element in colAccessory:
        if element.Location:
            FamilyName = element.Symbol.Family.Name
            if FamilyName not in elements: elements.append(FamilyName)
    elements_to_workset = cheklist(elements)
    change_element_workset(colAccessory, worksets_id, elements_to_workset, workset_todo_id)
    
elif  project_todo == 'Оборудование':
    elements = []
    for element in colEquipment:
        if element.Location:
            FamilyName = element.Symbol.Family.Name
            if FamilyName not in elements: elements.append(FamilyName)
    elements_to_workset = cheklist(elements)
    change_element_workset(colEquipment, worksets_id, elements_to_workset, workset_todo_id)
    
    
 
elif project_todo == 'Системы трубопроводов':
    systems = []
    for element in colPipeCurves:
        if element.Location:    
            if element.LookupParameter('Имя системы'):
                system = element.LookupParameter('Имя системы').AsString()
                if system not in systems: systems.append(system)
    systems_to_workset = cheklist(systems)
    change_system_workset(colPipeCurves, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colPipeFittings, worksets_id, systems_to_workset, workset_todo_id)
    change_system_workset(colPipeAccessory, worksets_id, systems_to_workset, workset_todo_id)

elif  project_todo == 'Арматура трубопроводов':
    elements = []
    for element in colPipeAccessory:
        if element.Location:
            FamilyName = element.Symbol.Family.Name
            if FamilyName not in elements: elements.append(FamilyName)
    elements_to_workset = cheklist(elements)
    change_element_workset(colPipeAccessory, worksets_id, elements_to_workset, workset_todo_id)
